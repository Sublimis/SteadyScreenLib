...
public class SteadyScreen
{
	...
	public static final String ServicePermission = "com.sublimis.steadyscreen.permission.ACTION_MOVE";
	...
	protected final BroadcastReceiver Receiver = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			if (intent != null && ActionMove.equals(intent.getAction()))
			{
				...
				final double xRaw = intent.getDoubleExtra(ParamMoveCoordX, Double.NaN);
				final double yRaw = intent.getDoubleExtra(ParamMoveCoordY, Double.NaN);

				final int xScr = Double.isNaN(xRaw) ? Integer.MIN_VALUE : (int) xRaw;
				final int yScr = Double.isNaN(yRaw) ? Integer.MIN_VALUE : (int) yRaw;
				...
				final boolean postUndo = isUndoCheckNeeded(timestamp);

				for (final View view : ViewsTmp)
				{
					performSteadyViewAction(view, xScr, yScr, postUndo);

					if (view instanceof ISteadyView iSteadyView)
					{
						iSteadyView.steadyViewAction(xRaw, yRaw);

						if (isMetaInfoNeeded(timestamp))
						{
							iSteadyView.steadyScreenMetaInfo(intent);
	...
	@MainThread
	protected void performSteadyViewAction(@NonNull final View view, final int x, final int y, final boolean postUndo)
	{
	...
	protected void postUndo(@NonNull final View view)
	{
